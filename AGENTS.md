...
- Treat Diffmogger as a work generator, not a blocker detector. If tickets remain, the scheduler should produce repair, setup, mock, fixture, defer, split, reframe, review, documentation, or alternate-ticket work unless a real safety boundary says otherwise.
- Treat blockers as planning inputs, not automation stop states. Current compatibility statuses such as `BLOCKED_ON_USER` and `BLOCKED_ON_ENVIRONMENT` annotate state; they are not a reason to freeze useful independent work.
...
- Do not treat today's status names, runner shape, scheduler internals, notifier bridge, or code-indexing implementation as permanent. They may be replaced when a migration makes the kit simpler and more reliable.
