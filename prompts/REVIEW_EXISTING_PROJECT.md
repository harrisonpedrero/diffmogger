...
- human bridge mode and notifier fallback behavior
...
The recurring automation prompt must distinguish structured human replies from freeform commands. If the human asks to be texted, messaged, or sent a status update, the automation should use `POST http://127.0.0.1:8765/api/notify` when available and should record `NOTIFIER_UNREACHABLE` in `docs/HUMAN_OUTBOX.md` when unavailable.
