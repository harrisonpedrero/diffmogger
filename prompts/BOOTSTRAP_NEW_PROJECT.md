...
- Create or update local automation helper scripts under `.diffmogger/scripts/`: `run_temporal_worker.sh`, `orchestration_cli.py`, `run_role_automation.sh`, `integrate_role_outputs.py`, `list_deferred_patches.py`, `run_observatory.py`, `repair_environment.py`, `state_brief.py`, `acquire_codex_lock.sh`, `release_codex_lock.sh`, `spawn_worker_agent.sh`, `summarize_worker_outputs.py`, and `compact_agent_state.py`.
