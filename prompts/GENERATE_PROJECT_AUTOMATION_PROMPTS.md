...
Generated prompts must distinguish local fallback data from requested source-backed data. Fixtures, mocks, seed data, and dry-run modes are allowed for safe local milestones, but if the intake depends on real, live, public, official, or third-party data, the generated ticket queue and run guidance must include source-mode strategy, connector/cache work, provenance/freshness receipts, unavailable-source states, no-fiction guardrails, and docs. Missing credentials or unavailable external services create setup/defer/alternate-validation work rather than a claim that fixture-only behavior is complete.
