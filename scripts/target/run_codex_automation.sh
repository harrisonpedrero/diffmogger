...
load_codex_automation_env() {
  ...
  if [ -f "$runtime_script_dir/load_automation_env.py" ]; then
    exec python3 "$runtime_script_dir/load_automation_env.py" --target "$TARGET" -- "${BASH:-bash}" "$0" "$@"
  fi
...
configure_diffmogger_browser() {
  ...
  if [ -z "${DIFFMOGGER_BROWSER_PATH:-}" ] && [ -z "${CHROME_PATH:-}" ] && [ -f "$runtime_script_dir/diffmogger_browser.py" ]; then
...
maybe_finalize_ticket_campaign() {
  if [ -f "$runtime_script_dir/ticket_run.py" ]; then
    python3 "$runtime_script_dir/ticket_run.py" . should-halt --finalize
    return "$?"
  fi
  return 1
}
...
staged_single_lane_unsafe_paths() {
  git diff --cached --name-only -z | python3 - <<'PY'
import sys
...
if [ "$exit_code" != "0" ] && [ "$exit_code" != "124" ] && [ -f "$runtime_script_dir/repair_environment.py" ]; then
  repair_status=0
  python3 "$runtime_script_dir/repair_environment.py" . \
