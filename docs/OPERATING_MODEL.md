...
Generated targets use the continuous DAG action scheduler with planner, builder, hardener/reviewer, and integrator prompt bases. The dashboard **Start** button launches one detached local runner for `.diffmogger/scripts/run_conveyor_automation.sh`; that stable wrapper enters the DAG runtime. **Stop** terminates the recorded runner process group. The runner records canonical execution DAG state in `.diffmogger/runtime/orchestration.sqlite3`, regenerates `.diffmogger/runtime/canonical_state_brief.md` before Codex runs, and generates `.diffmogger/runtime/automation_runner.json` and `.diffmogger/runtime/automation_conveyor_state.json` only as read-model projections. The scheduler chooses ready nodes and waves by DAG readiness, confidence, leases, and risk. Diffmogger is a work generator, not a blocker detector: blockers are planning inputs that create repair, setup, mock, fixture, defer, split, reframe, review, documentation, or alternate-ticket work while tickets remain. Typed state includes an append-only event ledger, execution DAG nodes for `orchestrate`, `decompose`, `scope`, `build`, `review`, `validate`, `repair`, `integrate`, `audit`, `calibrate`, blockers, and completion, plus hard/advisory DAG edges, repository capability manifests, checkpoints, current task/run projections, open blockers, validation receipts, active role runs, and bounded future decision projections for local observability.
...
Before applying full-suite-required patches, the integrator records clean-HEAD baseline verification in `.diffmogger/runtime/baseline_verification.json`. A failing baseline creates repair/setup/defer DAG work for normal hardener/finalization patches, but does not reject unrelated builder patches whose focused checks pass. Required check failures create repair work; missing tools create setup/harness work; missing project-local services become local harness/setup/mock/fixture work; external service failures become mock/local-fixture/defer work; browser or MCP failures become alternate validation or deferred QA work. Use `BLOCKED_ON_ENVIRONMENT` only as an annotation after creating that follow-up work. Patches that intentionally repair the baseline should declare `Verification scope: baseline_repair`.
...
Use `ACTIVE_WITH_PENDING_USER_INPUT` when a human request is open. It is an annotation that input exists, not a pause state.

Use `BLOCKED_ON_USER` only as an exceptional annotation. It does not pause scheduling; if tickets remain, create repair, setup, mock, defer, split, reframe, review, documentation, or alternate-ticket work.

Use `BLOCKED_ON_ENVIRONMENT` only as an exceptional annotation for local setup, permissions, dependencies, or external service configuration. It does not pause scheduling; create local setup, harness, mock, fixture, or defer DAG work and continue independent tickets.

Use `CRITICAL_STOP` only when continuing autonomously could be unsafe, destructive, or corrupting.

Failed validation follows the same liveness rule. Required checks create repair work, missing tools create setup/harness work, external service failures create mock/local-fixture/defer work, browser or MCP failures create alternate validation or deferred QA work, and repeated failures create planner split/reframe/defer work.
