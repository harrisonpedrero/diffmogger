...
The dashboard can add supplemental context files at setup time. In new sidecar targets those files are copied under `.diffmogger/context/` and tracked in the intake/dashboard state.
...
Use `examples/ticket-campaign/project_intake.md` to exercise bounded ticket-campaign mode. It checks that generated automation uses ticket-run phases, treats the dashboard-backed SQLite ticket queue as scope, starts actual automation directly after scaffold, supports dependency-aware `depends_on` ordering, and avoids open-ended roadmap language.
