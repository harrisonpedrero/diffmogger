...
Canonical runtime/task state belongs in `target/orchestration.sqlite3`; typed conveyor stage contracts, the current work item, repository capability manifest, validation receipts, blockers, and next actions live there. `target/canonical_state_brief.md` is the generated agent-readable view. Markdown and JSON files are prompt inputs, handoff surfaces, authored import/export surfaces, projections, exports, or migration aids.
