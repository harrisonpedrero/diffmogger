...
function conveyorModel(snapshot: ProjectSnapshot | null): HomeModel["conveyor"] {
  ...
  const lanes = LANE_ORDER.map((role) => {
    ...
    return {
      ...
      reason: text(fromSnapshot.reason ?? latest.reason, activeRole === role ? text(active.reason, "Lane is active.") : "Awaiting activity scheduler decision."),
      counts: {
...
function eventLedger(snapshot: ProjectSnapshot | null): HomeModel["eventLedger"] {
  ...
  return {
    rows: rows.slice(0, 8),
    emptyMessage: "No command, queue, activity scheduler, or commit events are exposed in the current snapshot.",
  };
}
